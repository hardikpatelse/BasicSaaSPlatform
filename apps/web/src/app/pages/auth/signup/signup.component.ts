    ...
    <div class="min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
      <div class="max-w-md w-full space-y-8">
        ...
        <form class="mt-8 space-y-6" (ngSubmit)="onSubmit()">
          ...
          <div *ngIf="selectedPlan" class="bg-indigo-50 border border-indigo-200 rounded-md p-4">
            <div class="flex">
              ...
              <div class="ml-3">
                <p class="text-sm text-indigo-700">
                  Selected plan: <strong>{{ selectedPlan === 'starter' ? 'Starter ($9/mo)' : 'Pro ($29/mo)' }}</strong>
                </p>
              </div>
            </div>
          </div>
          ...
          <div>
            <button type="submit" 
              ...
              <span class="absolute left-0 inset-y-0 flex items-center pl-3">
                <svg *ngIf="!loading" class="h-5 w-5 text-indigo-500 group-hover:text-indigo-400" fill="currentColor" viewBox="0 0 20 20">
...
export class SignupComponent {
  ...
  ngOnInit() {
    // Check if user came from pricing page with plan selection
  ...
  async onSubmit() {
    ...
    try {
      ...
      if (error) {
        ...
        this.message = 'Check your email for a confirmation link to complete your registration.';
        // Reset form
        this.formData = {
          fullName: '',
          email: '',
  ...
  async signUpWithGoogle() {
    ...
    try {
      ...
      this.error = error.message || 'An error occurred during Google sign up';
      this.loading = false;
    }
  }
}